...
local grab_script_modes = {
	...
	{"List (Animations)","Animations", subtype = "List"},
	{"Examine (Animations)","Animations", subtype = "Examine"},
...
local Game = {
	...
	Memory = { -- 1 USA, 2 Europe, 3 Japan, 4 Kiosk, 5 LodgeNet
		...
		animation_table_pointer = {0x7FBB50,nil,nil,nil,nil},
		-- 1000 0000 - ????
...
--------------------------------
-- Object > Player Statistics --
---- Written by theballaam96 ---
--------------------------------
function calculateXZDist(lx,lz,lr)
	lr = lr or 0;
...
function calculateXYZDist(lx,ly,lz,lr,lh)
	lr = lr or 0;
	lh = lh or 0;
	local playerObject = Game.getPlayerObject();
	if isRDRAM(playerObject) then
		local dxz = calculateXZDist(lx,lz,lr);
...
obj_model2 = {
	...
	object_types = { -- "-" means that spawning this object crashes the game
		...
		[0x241] = "Torch Light Source", -- Internal name is "torches"
...
local function getExamineDataLoadingZone(base)
	...
	if isRDRAM(base) then
		...
		local x_p = mainmemory.read_s16_be(base + loading_zone_fields.x_position);
		local y_p = mainmemory.read_s16_be(base + loading_zone_fields.y_position);
		local z_p = mainmemory.read_s16_be(base + loading_zone_fields.z_position);
		local lz_radius = mainmemory.read_u16_be(base + loading_zone_fields.radius);
		local lz_height = mainmemory.read_s16_be(base + loading_zone_fields.height);
...
-- Animations --
----------------
animation_labels = { -- TODO: These are probably different on Kiosk and maybe different on PAL/J
	[0x00] = "Idle (DK, Normal)",
	[0x01] = "Walking (DK)",
	[0x02] = "Creeping (DK)",
	[0x03] = "Running (DK)",
...
function Game.populateAnimationPointers()
	object_pointers = {};
...
local function drawGrabScriptUI()
	...
	if string.contains(grab_script_mode, "Animations") then
		Game.populateAnimationPointers();
	end
	...
	if #object_pointers > 0 and object_index <= #object_pointers then
		...
		if grab_script_mode == "List (Animations)" then
			pagifyThis(object_pointers, 40);
			for i = page_finish, page_start + 1, -1 do
				local animationBase = object_pointers[i];
				local animationNext = object_pointers[i+1];
				local animationName = Game.getAnimationName(i);
				local color = nil;
				if object_index == i then
					color = colors.green;
				end
				local size = 0;
				if animationNext ~= nil then
					size = animationNext - animationBase;
				end
				gui.text(gui_x, gui_y + height * row, toHexString(animationBase).." "..animationName.." - "..(i-1).." ("..toHexString(size)..")", color, 'bottomright');
				row = row + 1;
			end
		end
