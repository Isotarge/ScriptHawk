...
// BUGS & AIMS FOR V1.2
// (Just because something is listed here doesn't mean it'll make it in for 1.2. Solution may be too tough)

// Position Storage Updates
	// Storing camera data (8 votes)
		// Delving into char change obj
	// Storing the "stored position" (5 votes)
// Set-up Takeoff skip button (8 votes)
	// Set all key turned in flags
	// Set "Talked to K-Lumsy"
	// Clear key 3 & 8 turned in
	// Clear Rainbow Coin FTT
	// Set Key 3 & 8 flags
// ISG Timer Watch (4 votes)
// H. Velocity Watch (3 votes)
// Cumulative Lag Counter Watch (3 votes)
	// D-Up to reset to 0
// Update Lag Watch to check actual lag rather than lag boost
...
// MIPS ASM
[ReturnAddress]: 0x807FFFD0
[ReturnAddress2]: 0x807FFFD4
[ReturnAddress3]: 0x807FFFD8 // Compact Functions
...
[VariableDisplayArray]: 0x807FF908 // 0x14
...
PositionSavestates:
	...
	LoadPosition:
		...
		LW 		a2, 0x0 (a3)
		SW 		a2, 0xA4 (a1)
		// Camera - Loading the camera position back up causes problems. If you spam D-Left it kinda gets back, but it's not liking it so far
		// LW 		a1, @SwapObject
		// LI 		a3, @SavedCameraPosition
		// LW 		a0, @Camera
		// Li 		t9, @StaticCamera
		// LW 		a2, 0x0(a3)
		// SW 		a2, 0x210 (a1)
...
VariableDisplay:
	...
	VDispISG:
		...
		MTC1 	a2, f6 
		LI 		a2, 0x41AF4B60 // 21.9118
		MTC1 	a2, f2
		LI 		a3, 0x3089705F // 10^-9
		MTC1 	a3, f4
		LI 		a3, 0x42BC3891 // Minor Overflow Addition
		MTC1 	a3, f7
		MTC1 	r0, f8 // r0
	...
	VDispSpd:
		...
		LUI 	a2, 0x3EFF // f32 - 0.498 (Close to 0.5)
		MTC1 	a2, f1
		SUB.S 	f2, f0, f1 // Spd - 0.498 
		CVT.W.S f3, f2 // Integer component
		CVT.S.W f4, f3
		SUB.S 	f5, f0, f4 // Decimal component
		LUI 	a2, 0x4270
		MTC1 	a2, f1
		MUL.S 	f0, f4, f1 // Int * 60
		ADD.S 	f0, f0, f5 // (Int * 60) + Dec
